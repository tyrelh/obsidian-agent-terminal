...
var VIEW_TYPE_AGENT_TERMINAL = "agent-terminal:view";
...
var AgentTerminalView = class extends import_obsidian.ItemView {
  ...
  buildToolbar() {
    ...
    this.profileSelectEl.addEventListener("change", () => {
      this.applyProfile(this.profileSelectEl.value);
    });
  ...
  async startCommand(command) {
    if (!command) {
      new import_obsidian.Notice("Agent Terminal: command is empty.");
      return;
    }
    this.stopProcess(false);
    const cwd = this.plugin.resolveWorkingDirectory();
    if (!cwd) {
      new import_obsidian.Notice("Agent Terminal: could not resolve working directory.");
      return;
    }
    const settings = this.plugin.settings;
    const env = {
    ...
    if (process.platform === "win32") {
      ...
      if (!pluginDir) {
        new import_obsidian.Notice("Agent Terminal: plugin directory is not available.");
        return;
      }
      executable = "python3";
      const bridgePath = (0, import_node_path.join)(pluginDir, "scripts", "pty_bridge.py");
    ...
    try {
      ...
      if (process.platform !== "win32") {
        ...
        if (control && typeof control.write === "function") {
          this.processControlStream = control;
        }
      }
...
`);
      ...
      child.on("error", (error) => {
        this.terminal?.write(`\r
\x1B[31m[spawn error] ${error.message}\x1B[0m\r
`);
      ...
      child.on("exit", (code, signal) => {
        ...
        this.terminal?.write(`\r
\x1B[90m[process exited: ${details}]\x1B[0m\r
`);
      ...
      this.terminal?.write(`\r
\x1B[31m[start error] ${message}\x1B[0m\r
`);
      this.statusEl.setText("error");
    }
  }
  stopProcess(showNotice = true) {
    ...
    try {
      ...
      this.statusEl.setText("idle");
      if (showNotice) {
        new import_obsidian.Notice("Agent Terminal: process stopped.");
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      this.terminal?.write(`\r
\x1B[31m[stop error] ${message}\x1B[0m\r
`);
    }
  }
  sendResize() {
...
var AgentTerminalSettingTab = class extends import_obsidian.PluginSettingTab {
  constructor(plugin) {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian.Setting(containerEl).setName("Default profile").setDesc("Preselect a profile when opening Agent Terminal.").addDropdown((dropdown) => {
      dropdown.addOption("codex", "Codex CLI").addOption("claude", "Claude Code").addOption("custom", "Custom").setValue(this.plugin.settings.defaultProfile).onChange(async (value) => {
        this.plugin.settings.defaultProfile = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Codex command").setDesc("Command used by the Codex profile.").addText((text) => {
      text.setPlaceholder("codex --no-alt-screen").setValue(this.plugin.settings.codexCommand).onChange(async (value) => {
        this.plugin.settings.codexCommand = value.trim();
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Claude command").setDesc("Command used by the Claude profile.").addText((text) => {
      text.setPlaceholder("claude").setValue(this.plugin.settings.claudeCommand).onChange(async (value) => {
        this.plugin.settings.claudeCommand = value.trim();
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Custom command").setDesc("Command used by the Custom profile.").addText((text) => {
      text.setPlaceholder("your-command-here").setValue(this.plugin.settings.customCommand).onChange(async (value) => {
        this.plugin.settings.customCommand = value.trim();
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Working directory").setDesc("Leave empty to use your vault root directory.").addText((text) => {
      text.setPlaceholder("(vault root)").setValue(this.plugin.settings.workingDirectory).onChange(async (value) => {
        this.plugin.settings.workingDirectory = value.trim();
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian.Setting(containerEl).setName("Shell path").setDesc("Used on macOS/Linux to launch commands in a login shell.").addText((text) => {
      text.setPlaceholder("/bin/zsh").setValue(this.plugin.settings.shellPath).onChange(async (value) => {
...
var AgentTerminalPlugin = class extends import_obsidian.Plugin {
  constructor() {
    super(...arguments);
  ...
  async activateView(profileToStart) {
    ...
    if (!leaf) {
      ...
      if (!leaf) {
        new import_obsidian.Notice("Agent Terminal: unable to open a workspace leaf.");
        this.pendingInitialProfile = null;
        return;
      }
